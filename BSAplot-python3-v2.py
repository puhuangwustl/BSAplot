...
F_fai='genomev2.1/Sviridis_500_v2.0.fa.fai'    # faidx indexed genome size file
F_anno='genome/Sviridis_311_v1.1.annotation_info.txt'    # gene annotation file, phytozome format
F_anno='genomev2.1/Sviridis_500_v2.1.annotation_info.txt'    # gene annotation file, phytozome format

# genome size dic
f=open(F_fai, encoding='utf-8')
...
app.layout = html.Div(children=[
	...
	dcc.Graph(id='BSA_plot'), # animation not good for this application. ,animate=True),
	html.Div([
			html.Div([
...
def update_bsaplot(WIN,LOW,contents,fname):
	...
	if fname.endswith('.gz'):
		...
		infile=base64.b64decode(contents.split(',')[1]).decode().splitlines()
	if len(infile)>MAXSNP:
		return go.Figure(layout=bsa_layout('Warning, too large file for analysis, consider thinning data'))
