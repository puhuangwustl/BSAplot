...
app.layout = html.Div(children=[
	...
	html.Div([
		dt.DataTable(
			...
			# Initialise the rows
			rows=[{}],
			columns=[ 'Pos','Mut/Total','Annotation','Impact','Codon','Allele','Gene','AtLog','AtName','AtAnno','Transcript'],
...
def update_figure(WIN,LOW,contents,fname):
	...
	return go.Figure(
		...
		layout=go.Layout(title='BSA plot for "'+fname+'"',
			...
			xaxis=dict(zeroline=False,title='Genomic position',range=(0,d[3][-1]+1000000)),
...
def display_selected_data(selectedData):
	...
	for point in points:
		if point[u'curveNumber']==1 :
			text=point[u'text']
	newtexts=re.split('<br>',text)
	out=[]
	for item in newtexts[2:]:
		dic={} 
		anno=item.replace('(','|').replace(')','|').split('|')
		# SNPeff new version got different format, be cautious
		dic['Pos']=newtexts[0]
		dic['Mut/Total']=newtexts[1].split()[0]
		dic['Annotation']=anno[0]
		dic['Impact']=anno[1]
		dic['Type']=anno[2]
		dic['Codon']=anno[3]
		dic['Allele']=anno[4]
		dic['Gene']=anno[6]
		dic['Transcript']=anno[9]
		out.append(dic)
		if dic['Gene']:
			cmd='grep '+dic['Gene']+' '+F_anno+' |  head -1 | cut -f11-'
			geneanno=commands.getstatusoutput(cmd)[1].split('\t')
			dic['AtLog']=geneanno[0]
			dic['AtName']=geneanno[1]
			dic['AtAnno']=geneanno[2]
			dic['OsLog']=geneanno[3]
			dic['OsName']=geneanno[4]
		else:
			dic['AtLog']=None
			dic['AtName']=None
			dic['AtAnno']=None
