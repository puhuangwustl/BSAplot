...
# genome size dic
f=open('/media/Setaria_1/Share/Sviridis_vphytozome1.1/Sviridis_311_v1.0.fa.fai')
...
d=[]
ch=0
with gzip.open('mut.eff.vcf.gz', 'rb') as f:
	for l in f:
		if l.startswith("#"):
			continue
		if l.startswith('scaffold'):
...
figure=go.Figure(
	...
	layout=go.Layout(title='BSA plot',yaxis=dict(range=(0,YMAX)),shapes=[
		dict(type='rect',x0=startdic[ch][0],x1=startdic[ch][1],y0=0,y1=YMAX,fillcolor=startdic[ch][2],line=dict(width=0)) for ch in chs
		]),
)
...
app.layout = html.Div(children=[
	html.H1(children='BSA plot for Setaria viridis'),
	dcc.Graph(
		figure=figure,
		id='BSA plot'
	),
